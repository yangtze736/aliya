...
#
# Script Name: auto-deployments.sh
#
# Description: modify from Classic Shell Scripting
#	Arnold Robbins and Nelson H. F. Beebe
#
# Created: 2015年09月24日 星期四 09时44分52秒
...
build_one()
{
	...
	# Copy the package file to the remote host
	...
	now="`date $DATEFLAGS`"
	logfile="${package}_${host}_${now}.log"

	## echo "DEBUG: LOGFILE  = $LOGDIR/${package}_${host}_${now}.log"
...
find_package()
{
	...
	do
		...
		do
			...
			find_file $srcdir/$1.tar.gz		"tar xfz"	&& return
			find_file $srcdir/$1.tar.Z		"tar xfz"	&& return
			find_file $srcdir/$1.tar		"tar xf"	&& return
			find_file $srcdir/$1.tar.bz2	"tar xfj"	&& return
			find_file $srcdir/$1.tgz		"tar xfz"	&& return
			find_file $srcdir/$1.zip		"unzip -q"	&& return
			find_file $srcdir/$1.jar		"jar xf"	&& return
		done
	done
}
...
# test -x $MAIL || echo "Cannot find mail client"
